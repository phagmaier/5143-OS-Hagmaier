'''
Author: Proffesor Griffin

RandInstructions class is what generates all the .exe files which we will use as our instructions for each run
we have to randomly generate loads adds,subs,divs,reads,and writes, while making sure we don't skip a prioirty instruction number
as well as create shared instructions that always read and write to the same memory adress 
User input determines how many of these files are generated and each file should have 1000 instruction (each instruction is a list [])
(not counting sleeps) 
'''
...
class RandInstructions:
    ...
    #ri = RandInstructions(privilegedRatio=0.3, sleepRatio=0.15, numProcesses=3)
