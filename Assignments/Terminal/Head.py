'''
 ...
 Description:  Head is a function that if '-n' is not passed as a flag then by default the first 10 lines of the 
...
def head(waspiped=None, *args):
	...
	if len(files) > 0:
		for i in files:
			with open(i, 'r') as headfile:
				length = headfile.readlines()
				headfile.close()
				length = len(length)
				if length < number:
					number = int(length)
	...
	if pipeOrRe == False:
		...
		return linesIfPiped
#head(None, False, 'aa.txt')
#head(True, False, ['Hello', 'Parker'])
